	...
	protected void timerTick() {
		...
		for (Creature creature : creaturesCopy) {
			...
			if(creature instanceof Food) {
				Snake.setFoodX(creature.getX());
				Snake.setFoodY(creature.getY());
			}
//			if(!(creature instanceof Snake))    // Reset current creature's map position (its position will be marked again, if it still lives)
				updateCreaturesMap(creature.getX(), creature.getY(), null);
			//			else {
			//				Snake s = (Snake)creature;        	
			//				for(int i =0;i<s.getSize();i++) {      	
			//					updateCreaturesMap(s.getListX().get(i), s.getListY().get(i), null);
			//				}
			//			}
			// Choose action
			...
			if (action != null) {
				if (action.getType() == Action.Type.REPRODUCE) {
					...
					if(creature instanceof Snake){
						Snake s = (Snake)creature;        							      	
							updateCreaturesMap(s.getListX().get(0), s.getListY().get(0), null);					
					}
					...
					if (attackedCreature != null) {
						...
						int x = (int) (Math.random() * super.getGridWidth());
						int y = (int) (Math.random() * super.getGridHeight());
						
						while(!(isPositionFree(x,y)==true)) {
							x = (int) (Math.random() * super.getGridWidth());
							y = (int) (Math.random() * super.getGridHeight());
						}						
						Food h = new Food(x,y);												
						addCreature(h);
						Snake.setFoodX(h.getX());
						Snake.setFoodY(h.getY());
			...
			if(!(creature instanceof Snake))    // Reset current creature's map position (its position will be marked again, if it still lives)
				...
				Snake s = (Snake)creature;        	
				for(int i =0;i<s.getSize();i++) {      	
					updateCreaturesMap(s.getListX().get(i), s.getListY().get(i), creature);
				}
			}
		}
	}
